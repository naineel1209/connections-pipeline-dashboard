...
export const defaultMessageTemplate = `Hi,
...
export function renderMessageTemplate(template: string, connection: Connection, profile: Profile): string {
  ...
  return template.replace(/\{(name|sender)\}/g, "").replace(/\{(company|job|joblink|headline)\}/g, (_, key: keyof typeof values) => values[key]);
}
